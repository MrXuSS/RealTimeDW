...
import com.haiyi.entity.Table_HS_LJFMX
 ...
 *
 */
object FlinkKuduCEP2 {
  def main(args: Array[String]): Unit = {
    val env: StreamExecutionEnvironment = StreamExecutionEnvironment.getExecutionEnvironment
    env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime)
    env.setParallelism(5)

    val stream: DataStream[Table_HS_LJFMX] = env.addSource(new MyKuduSource).map(JSON.parseObject(_, classOf[Table_HS_LJFMX]))
      .assignAscendingTimestamps(_.dfny.toLong+System.currentTimeMillis())

    val dateFormat = new SimpleDateFormat("yyyyMM")

    val cepPattern: Pattern[Table_HS_LJFMX, Table_HS_LJFMX] = Pattern.begin[Table_HS_LJFMX]("begin")
      .where {
        obj => {
          val date: Date = dateFormat.parse(obj.dfny)
    ...
    val patternStream: PatternStream[Table_HS_LJFMX] = CEP.pattern(stream.keyBy("gzdbh", "jldbh", "ywlbdm", "dlfddm", "fklxdm", "jjlxdm", "dflbdm", "sdlx"), cepPattern)

    val resultStream: DataStream[Table_HS_LJFMX] = patternStream.select((map: Map[String, Iterable[Table_HS_LJFMX]]) => {
      val hS_LJFMX: Table_HS_LJFMX = map.getOrElse("begin", null).iterator.next()
      hS_LJFMX
    })
  ...
  class MyKuduSource extends RichParallelSourceFunction[String]{
    ...
    private val tableName:String = "HS_LJFMX"
